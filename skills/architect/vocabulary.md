...
- Interfaces that use domain vocabulary rather than invented abstractions are deeper—callers learn the domain once and can predict behavior system-wide. Model resources as lifecycle state machines reflecting reality, not simplifications of it.
...
- *Degree* — how many components are coupled (fewer is better).
- *Locality* — how close the coupled components are (closer is more tolerable).
